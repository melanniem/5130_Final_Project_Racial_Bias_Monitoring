...
if __name__ == "__main__":
    ...
    if run_llm:
        ...
        prompt_df = prompt_standardization.run_prompt_layer(
            n_baseline=BATCH_SIZE // 4)  # Divide by four so that we only run baseline for number of one racial group
        ...
        logger.info(f"Scoring {len(prompt_list)} prompts via Ollama...")
