...
INSTRUCTIONS:
Evaluate this candidate's fit for the role taking into consideration qualifications, skills, and experience.
...
#
#     return baseline_df
def build_null_baseline_prompts(input_df: pd.DataFrame, k: int = 10) -> pd.DataFrame:
    """
    Generate null baseline prompts: 1 per job title × k repetitions.
    Name is stripped from resume text and replaced with 'Applicant'.
    Each repetition gets a unique negative name_id for persistence.
    """
...
def run_prompt_layer(n_baseline: int = 10):
    input_df = load_and_built_prompt()
    verify_prompt(input_df)

    # Generate and append null baseline rows
    baseline_df = build_null_baseline_prompts(input_df, k=n_baseline)
    input_df = pd.concat([input_df, baseline_df], ignore_index=True)
    print(f"Appended {len(baseline_df)} null baseline prompts ({n_baseline} reps × {baseline_df['job_title_id'].nunique()} jobs).")
